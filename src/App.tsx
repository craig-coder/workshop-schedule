...
// All sync (GET/POST) goes through your Vercel API proxy
const SYNC_URL = "/api/sheets";

// Change this to your tracker tab URL if needed
...
const SECTION_DEFS: Record<string, string[]> = {
  ...
  "Top Coat": ["Prep", "Side 1.1", "Side 2.1", "Prep 2", "Side 2.1", "Side 2.2"],
  "Wrap & Pack": ["Cabinets Wrapped", "Rails Cut", "Doors Wrapped", "Fitters Kit Checked", "Loaded"],
  Remedials: [],
  "Job Complete": [],
};
const STAGE_COLUMNS = Object.keys(SECTION_DEFS) as (keyof typeof SECTION_DEFS)[];

/* -------------------- Local persistence for UI progress -------------------- */
...
export default function App() {
  ...
  const [zoom, setZoom] = React.useState<number>(() => {
    const saved = Number(localStorage.getItem("ui_zoom") || "1");
    return isNaN(saved) ? 1 : saved;
  });
  React.useEffect(() => {
    localStorage.setItem("ui_zoom", String(zoom));
  }, [zoom]);
  ...
  function cleanText(s?: string) {
    ...
    if (["true", "false", "1", "0", "✓", "✗"].includes(v.toLowerCase())) return "";
    return v;
  }

  function getJobKey(r: Record<string, string>) {
    // Prefer a stable ID if present, otherwise Client/Job merged
    const id = cleanText(r["ID"] || r["Job No"] || r["Order No"]);
    const clientOrJob = cleanText(r["Client"] || r["Job"] || r["Project"] || r["Job Name"]);
    if (id) return id;
    if (clientOrJob) return clientOrJob;
    return JSON.stringify({ Client: r["Client"] || "", Job: r["Job"] || "" });
  }
  ...
  async function pushUpdate(payload: any) {
    try {
      ...
      // ignore network hiccups so UI stays responsive
    }
  }

  /* ---------- Setters that also sync ---------- */
  ...
  /* ---------- Open a stage -> pull latest, then show modal row ---------- */
  ...
  /* ---------- Poll currently open job every 10s ---------- */
  React.useEffect(() => {
    if (!openKey) return;
    let cancelled = false;
  ...
  /* ---------- Optional global sync every 15s ---------- */
  React.useEffect(() => {
  ...
  function renderCell(h: string, r: Record<string, string>) {
    // Clickable stage cells -> open subsections
    ...
    // Special-case: render hyperlinks
    if (/link/i.test(h)) {
      const url = (r[h] || "").trim();
      if (!url) return null;
      return (
  ...
  /* -------------------- Render -------------------- */
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6" style={{ fontSize: `${zoom * 100}%` }}>
        ...
        <header className="flex items-center justify-between gap-4 flex-wrap">
          <div className="min-w-[260px]">
            ...
            <p className="text-sm text-gray-600">
              Click any stage cell to open sub-tasks. Orange = in progress, Green = complete.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap text-sm">
            ...
            {/* CSV URL + Refresh */}
            <input
              className="border rounded-lg px-3 py-2 w-[360px]"
              value={sheetUrl}
