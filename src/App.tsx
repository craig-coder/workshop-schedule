...
/** Normalize for matching with backend: lower-case, collapse spaces, trim */
function normKey(s: string) {
  return String(s || "").toLowerCase().replace(/\s+/g, " ").trim();
}

/** Treat boolean-ish values as not-part-of-title and normalize */
...
export default function App() {
  ...
  /** Map likely column names from your CSV */
  const colMap = {
    title: "Job", // visible “Job” column in your CSV
    status: "Status",
    start: "Start",
    end: "End",
  ...
  /* ---------- Job key (must match backend) ---------- */
  function getJobKey(r: Record<string, string>) {
    const id = cleanText(r[colMap.id] || r["Job No"] || r["Order No"]);
    const client = cleanText(r[colMap.client] || r["Customer"]);
    const title = cleanText(r[colMap.title] || r["Job Name"] || r[colMap.project]);
    // Prefer ID; else client — title; else title; else client
    const raw = id || (client && title ? `${client} — ${title}` : (title || client || ""));
    return normKey(raw);
  }
  ...
  async function pushUpdate(payload: any) {
    try {
      await fetch(SYNC_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" }, // JSON; Apps Script handles this now
        body: JSON.stringify(payload),
      });
    } catch {
      // keep UI snappy even if offline
    }
  }

  /* ---------- User actions -> push to server ---------- */
  function setSubStatus(
    ...
    // Use a human-friendly 'by' if present
    pushUpdate({
      job: jobKeyNorm,
      updatedBy: r["Assigned To"] || "Unknown",
      stage,
      subtask: name,
      status,
    });
  ...
  /* ---------- Open checklist: pull latest once ---------- */
  ...
  /* ---------- Global sync every 15s across all jobs ---------- */
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto grid gap-6">
        ...
        <section className="bg-white rounded-2xl shadow p-4">
          <div className="overflow-auto border rounded-xl">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {filtered.map((r, i) => {
                  const key = getJobKey(r); // normalized key
        ...
        <footer className="text-xs text-gray-500 text-center">
          Google Sheets is the source of truth. This app auto-syncs every 15s and when you open a checklist. Use “Sync now” to force a refresh.
        </footer>
      </div>
    </div>
  );
